...
Owners can create a new business from the dashboard. New businesses start as `PendingApproval`, create an active owner membership for the current user, and create a staff profile for the owner so the business can be configured immediately. Owners can manage the operational basics for their business: services, staff display names and active state, business weekly working hours, staff weekly working hours, pending appointment requests, approved upcoming appointments, and employee invitations. The first working-hours editor intentionally supports one interval per day; multiple intervals, breaks, holidays, and leave management remain part of the future scheduling upgrade.
